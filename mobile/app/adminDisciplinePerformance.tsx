...
export default function AdminDisciplinePerformance() {
  ...
  // ── Rows
  const [defaultMetric, setDefaultMetric] = useState("Discipline Score (0-100)");
  ...
  function updateRow(studentUserId: number, patch: Partial<DiscRow>) {
    ...
    try {
      await apiPost("/api/admin/performance/batch", {
        ...
        entries: validRows.map((r) => ({
          ...
          metric: r.metric || "Discipline Score",
          value: r.value,
          notes: r.notes,
        })),
      });
      Alert.alert("✅ Saved!", `Discipline entries saved for ${validRows.length} student(s).`);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        {sportId !== null && (
          <AppCard style={[styles.card, { marginHorizontal: 14, marginTop: 14 }]}>
            <Text style={[styles.sectionTitle, { color: theme.text }]}>Discipline Metric</Text>
            ...
            <TextInput
              ...
              placeholder="e.g. Discipline Score, Warning Points"
        ...
        {sportId !== null && (
          <AppCard style={[styles.card, { marginHorizontal: 14, marginTop: 0 }]}>
            <Text style={[styles.sectionTitle, { color: theme.text }]}>Student Records</Text>
            <Text style={[styles.subText, { color: theme.textSub }]}>
              Enter discipline values per student. Blank rows will be skipped.
            </Text>
          ...
          >
            {saving ? (
              ...
              <>
                ...
                <Text style={styles.saveText}>Save Discipline Entries</Text>
              </>
            )}
          </Pressable>
        )}
...
const styles = StyleSheet.create({
  ...
  avatarText: { color: "#F87171", fontSize: 16, fontWeight: "900" },
